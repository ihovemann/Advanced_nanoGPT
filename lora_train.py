...
if torch.cuda.is_available():
    ...
    device_type = "cpu"# note: float16 data type will automatically use a GradScaler
#ptdtype = {'float32': torch.float32, 'bfloat16': torch.bfloat16, 'float16': torch.float16}[dtype]
ptdtype = torch.float16
ctx = nullcontext() # poor man's data loader
...
if init_from == 'scratch':
    ...
    ckpt_path = 'out-shakespeare-baseline/ckpt.pt'
...
if master_process and not os.path.exists(csv_path):
...
if init_from == 'resume':
    pass #optimizer.load_state_dict(checkpoint['optimizer'])
...
if ddp:
    raise RuntimeError("DDP is not supported on MPS")
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        print(f"[CSV] logged step {iter_num} for {task_name}")
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                checkpoint = {
                    ...
                    'lora_params': get_peft_model_state_dict(raw_model),
